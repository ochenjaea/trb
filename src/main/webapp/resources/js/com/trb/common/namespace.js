...
var Namespace = (function() {
	...
	var _isHttpRequestSuccessful = function(status) {
		return (status >= 200 && status < 300)
				...
				(!status && (location.protocol == "file:" || location.protocol == "chrome:")); // Internet
																								// Explorer
																								// mangled
																								// the
																								// status
	 ...
	 * @public
	 * @param String
	 *            identifier The namespace string
	 * @param Object
	 *            klasses (OPTIONAL) An object which properties will be added to
	 *            the namespace
	...
	/**
	 * Maps an identifier to a uri. Is public so it can be overriden by custom
	 * scripts.
	 * 
	 * @public
	 * @param String
	 ...
	 *            identifier The namespace identifier
	 * @param Function
	 *            successCallback When set, the file will be loaded
	 *            asynchronously. Will be called when the file is loaded
	 * @param Function
	 *            errorCallback Callback to be called when an error occurs
	...
	/**
	 * Includes a remote javascript file identified by the specified namespace
	 * string. The identifier must point to a class. Separators in the string
	 * will be converted to slashes and the .js extension will be appended.
	 * 
	 * @public
	 * @param String
	 *            identifier The namespace string
	 * @param Function
	 *            callback (OPTIONAL) A function to call when the remote script
	 *            has been included
	 */
	...
	/**
	 * Imports properties from the specified namespace to the global space (ie.
	 * under window)
	 * 
	 * The identifier string can contain the * wildcard character as its last
	 * segment (eg: com.test.*) which will import all properties from the
	 * namespace.
	 * 
	 * If not, the targeted namespace will be imported (ie. if com.test is
	 * imported, the test object will now be global). If the targeted object is
	 * not found, it will be included using include().
	 * 
	 * @public
	 * @param String
	 *            identifier The namespace string
	 * @param Function
	 *            callback (OPTIONAL) A function to call when the process is
	 *            completed (including the include() if used)
	 * @param Boolean
	 *            autoInclude (OPTIONAL) Whether to automatically auto include
	 *            the targeted object is not found. Default is
	 *            Namespace.autoInclude
	 */
	_namespace.use = function(identifier) {
		var identifiers = _toArray(identifier);
		var callback = arguments[1] || false;
		var autoInclude = arguments.length > 2 ? arguments[2]
				: Namespace.autoInclude;
		var event = {
			'identifier' : identifier
		};
		...
		for ( var i = 0; i < identifiers.length; i++) {
			...
			if (target == '*') {
				// imports all objects from the identifier, can't use include()
				// in that case
						...
						if (callback) {
							_namespace.include(identifier, function() {
								...
								if (i + 1 < identifiers.length) {
									...
									_namespace.unpack(identifiers.slice(i + 1),
											callback, autoInclude);
								} else {
	...
	_namespace.from = function(identifier) {
		return {
			...
			/**
			 * Includes the identifier specified in from() and unpack the
			 * specified indentifier in _identifier
			 * 
			 * @see Namespace.use()
			 */
	...
	/**
	 * Adds methods to javascript native's object Inspired by
	 * http://thinkweb2.com/projects/prototype/namespacing-made-easy/
	 * 
	 * @public
	 */
	_namespace.registerNativeExtensions = function() {
		...
		/**
		 * @see Namespace.provide() Idea and code submitted by Nathan Smith
		 *      (http://github.com/smith)
		 */
...
/**
 * Base uri when using Namespace.include() Must end with a slash
 * 
 * @var String
 */
Namespace.baseUri = './';
